...
const transporter = nodemailer.createTransport({
    ...
    secure: parseInt(process.env.SMTP_PORT, 10) === 465, // true para porta 465, false para outras
    auth: {
...
const corsOptions = {
    origin: (origin, callback) => {
        // Em produção, permita apenas a origem do seu frontend.
        // Em desenvolvimento, permita requisições sem origem (ex: Postman) ou do localhost.
...
app.post('/api/auth/register', async (req, res) => {
    ...
    try {
        ...
        const verificationUrl = `${req.protocol}://${req.get('host')}/loja.html?verify=${verificationToken}`;
        const mailOptions = {
            to: email,
